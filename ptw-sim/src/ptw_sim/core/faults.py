...
@dataclass
class AccessFlagFault(BaseFault):
    """
    Access flag fault - access flag bit clear.

    This fault occurs when a valid descriptor is accessed but its
    AF bit is 0, usually notifying software to set it.

    ESR_ELn.DFSC/IFSC encoding for access flag faults:
        0b0010xx where xx = level
    """

    def __post_init__(self) -> None:
        if not self.message:
            self.message = (
                f"Access flag not set (AF=0) at Stage {self.stage} Level {self.level}"
            )
