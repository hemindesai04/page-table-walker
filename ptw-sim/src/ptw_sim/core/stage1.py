...
from ptw_sim.models.address import (
    VirtualAddress,
    IntermediatePhysicalAddress,
    PhysicalAddress,
    calculate_descriptor_address,
)
from ptw_sim.models.descriptor import (
    Descriptor,
    TableDescriptor,
    PageDescriptor,
    BlockDescriptor,
...
from ptw_sim.core.stage2 import Stage2Walker, Stage2WalkEvent, Stage2WalkResult
...
class Stage1Walker:
    ...
    def __init__(
        ...
        self.output_addr_bits = output_addr_bits
        self.s2_granule_kb = s2_granule_kb
    ...
    def walk(
        ...
        for level in range(self.starting_level, 4):
            ...
            table_ipa = IntermediatePhysicalAddress(current_table_ipa, granule_kb=self.s2_granule_kb)
            ...
            if descriptor.is_table:
                ...
                if (current_table_ipa >> self.output_addr_bits) != 0:
                    event.description = f"Stage 1 Level {level} - ADDRESS SIZE FAULT"
                    events.append(event)
                    return Stage1WalkResult(
                        success=False,
                        input_va=va,
                        events=events,
                        fault=FaultRecord(
                            fault_type=FaultType.ADDRESS_SIZE_FAULT,
                            stage=1,
                            level=level,
                            address=va.value,
                            message=f"Address requires {current_table_ipa.bit_length()} bits, but only {self.output_addr_bits} bits configured",
                            far_el1=va.value
                        )
                    )
                ...
                # Check address size
                if (final_ipa >> self.output_addr_bits) != 0:
                    event.description = f"Stage 1 Level {level} - ADDRESS SIZE FAULT"
                    events.append(event)
                    return Stage1WalkResult(
                        success=False,
                        ...
                        fault=FaultRecord(
                            ...
                            level=level,
                            address=va.value,
                            message=f"Address requires {final_ipa.bit_length()} bits, but only {self.output_addr_bits} bits configured",
                            far_el1=va.value
                        )
                    )

                # Check access flag
                if not data_desc.access_flag:
                    event.description = f"Stage 1 Level {level} - ACCESS FLAG FAULT"
                    events.append(event)
                    return Stage1WalkResult(
                        success=False,
                        input_va=va,
                        events=events,
                        fault=FaultRecord(
                            fault_type=FaultType.ACCESS_FLAG_FAULT,
                            stage=1,
                            level=level,
                            address=va.value,
                            message=f"Access flag not set (AF=0) at Stage 1 Level {level}",
                            far_el1=va.value
                        )
                    )
                ...
                return Stage1WalkResult(
                    success=True,
                    input_va=va,
                    output_ipa=IntermediatePhysicalAddress(final_ipa, granule_kb=self.s2_granule_kb),
                    events=events,
