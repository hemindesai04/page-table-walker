    ...
    def walk(
        ...
        for level in range(self.starting_level, 4):
            ...
            if descriptor.is_table:
                ...
                if (current_table_pa >> self.output_addr_bits) != 0:
                    event.description = f"Stage 2 Level {level} - ADDRESS SIZE FAULT"
                    events.append(event)
                    return Stage2WalkResult(
                        success=False,
                        input_ipa=ipa,
                        ...
                        fault=FaultRecord(
                            ...
                            level=level,
                            address=ipa.value,
                            message=f"Address requires {current_table_pa.bit_length()} bits, but only {self.output_addr_bits} bits configured",
                            far_el2=ipa.value
                        )
                    )
                ...
                if (final_pa >> self.output_addr_bits) != 0:
                    event.description = f"Stage 2 Level {level} - ADDRESS SIZE FAULT"
                    events.append(event)
                    return Stage2WalkResult(
                        success=False,
                        ...
                        fault=FaultRecord(
                            ...
                            level=level,
                            address=ipa.value,
                            message=f"Address requires {final_pa.bit_length()} bits, but only {self.output_addr_bits} bits configured",
                            far_el2=ipa.value
                        )
                    )

                if not data_desc.access_flag:
                    event.description = f"Stage 2 Level {level} - ACCESS FLAG FAULT"
                    events.append(event)
                    return Stage2WalkResult(
                        success=False,
                        input_ipa=ipa,
                        ...
                        fault=FaultRecord(
                            ...
                            level=level,
                            address=ipa.value,
                            message=f"Access flag not set (AF=0) at Stage 2 Level {level}",
                            far_el2=ipa.value
                        )
                    )
                ...
                return Stage2WalkResult(
                    success=True,
                    input_ipa=ipa,
                    output_pa=PhysicalAddress(final_pa, pa_bits=self.output_addr_bits, granule_kb=self.granule_kb),
                    events=events
                )
