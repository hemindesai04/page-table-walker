...
from ptw_sim.models.registers import (
    RegisterState,
    TTBR,
    TCR,
    VTCR,
...
class TCRConfig(BaseModel):
    ...
    TG0: int = Field(default=4)
    TG1: int = Field(default=4)
...
def build_register_state(config: ScenarioConfig) -> RegisterState:
    ...
    return RegisterState(
        ...
        tcr_el1=TCR(
            ...
            tg0=GranuleSize(config.registers.TCR_EL1.TG0 * 1024),
            tg1=GranuleSize(config.registers.TCR_EL1.TG1 * 1024),
        ),
        vtcr_el2=VTCR(
            t0sz=config.registers.VTCR_EL2.T0SZ,
            sl0=config.registers.VTCR_EL2.SL0,
            tg0=GranuleSize(config.registers.VTCR_EL2.TG0 * 1024),
        )
    )
...
def get_virtual_address(config: ScenarioConfig) -> VirtualAddress:
    ...
    is_upper = (va_value >> (va_bits - 1)) & 1 == 1
    granule_kb = config.registers.TCR_EL1.TG1 if is_upper else config.registers.TCR_EL1.TG0
