...
class TableDescriptor(Descriptor):
    ...
    def next_table_address(self) -> int:
        ...
        """
        # Mask to extract bits [51:12] (up to 52-bit PA)
...
class PageDescriptor(Descriptor):
    ...
    def output_address(self) -> int:
        ...
        The address is in bits [47:12] or up to [51:12] for FEAT_LPA.
        """
        return self.value & 0x000FFFFFFFFFF000
...
class BlockDescriptor(Descriptor):
    ...
    def output_address(self) -> int:
        ...
        if self.level == 1:
            ...
            return self.value & 0x000FFFFFFC000000
        elif self.level == 2:
            # 2MB block - bits [20:0] must be ignored/zero
            return self.value & 0x000FFFFFFFE00000
        return self.value & 0x000FFFFFFFFFF000
...
def build_table_descriptor(next_table_address: int) -> int:
    ...
    """
    # Ensure address is page-aligned and up to 52 bits
    aligned_addr = next_table_address & 0x000FFFFFFFFFF000
...
def build_page_descriptor(
    ...
    Returns:
        64-bit descriptor value.
    """
    aligned_addr = output_address & 0x000FFFFFFFFFF000
    value = aligned_addr
